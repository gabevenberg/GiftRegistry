...
def display_page(data):
    ...
    for i in range(len(data)):      
        ...
        desc.grid(row=i+row_offset, column=1)
        priority.grid(row=i+row_offset, column=2)
        qtyLeft.grid(row=i+row_offset, column=3)
        purchaseLink.grid(row=i+row_offset, column=4)
        ...
        purchaseButton.grid(row=i+row_offset, column=5)
    root.mainloop()    
    display=root
